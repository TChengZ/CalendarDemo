...
public class DateUtil {
	...
	/**
	 * 将日期格式转换成CustomDate
	 * @param dates 日期数组，每个元素必须是yyyy-MM-dd格式
	 * @return
	 */
	public static List<CustomDate> parseStringToCustomDate(String[] dates){
		List<CustomDate> list = new ArrayList<CustomDate>();
		try {
			if (dates != null) {
				for (int i = 0; i < dates.length; i++) {
					Date date = mSimpleDataFormat.parse(dates[i]);
					CustomDate customDate = new CustomDate(date.getYear() + 1900, date.getMonth() + 1, date.getDate());
					list.add(customDate);
				}
			}
		}
